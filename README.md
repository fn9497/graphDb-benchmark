...
- **`BENCH_ITERATIONS=20`** per read workload by default, not the suggested ≥100 — set
...
`PLATFORMS` in `scripts/common.py`, and re-run `scripts/run_all.sh`. See
...
- **Traversals:** 1/2/3-hop `MATCH` queries from randomly sampled start nodes,
  `BENCH_ITERATIONS` times each (default 20; raise for a more rigorous run).
...
- **Mixed workload:** `BENCH_CONCURRENCY` (default 10) concurrent threads, 80/20
...
*(Run `./scripts/run_all.sh` and paste the output of `results/REPORT.md` below —
...
3. If it doesn't speak Bolt/Cypher, add a small adapter module implementing the same
   `get_driver()` / `session.run()` surface used here (or port the four Cypher
...
4. Re-run `./scripts/run_all.sh` — the loader, workloads, warm-up, and report
